...
from api.utils.fakers import UserFactory, UserProfileFactory
...
class UserApiTests(APITestCase):
    ...
    def test_create_users(self):
        with self.subTest('staff registration success'):
        ...
        with self.subTest('staff registration failed'):
            response = self.client.post(
                '/api/users/register/',
                {
                    'email': self.staff_email, 'password': self.staff_pass, 'first_name': 'admin1'
        ...
        with self.subTest('customer registration success'):
            response = self.client.post(
                '/api/users/register/',
                {
        ...
        with self.subTest('customer registration failed'):
            response = self.client.post(
                '/api/users/register/',
                {
                    'email': self.customer_email, 'password': self.customer_pass, 'first_name': 'customer1'
